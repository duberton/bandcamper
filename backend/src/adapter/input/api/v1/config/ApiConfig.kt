...
val apiModule = module {
    single<ScrapeAlbumPagePort> {
        ScrapeAlbumPageUseCase(
            get(named(DSL_SKRAPE_NAME)), get(named(DYNAMODB_ALBUM_REPO)), get()
        )
    }
    single<FindAllAlbumsPort> { FindAllAlbumsUseCase(get(named(DYNAMODB_ALBUM_REPO))) }
